...
if(isset($_GET['PageName'], $_GET['PageDesc'], $_GET['ValidPage'])){
    ...
    if(is_dir($dir)){
        ...
        if(\Administration\app\Fopen::createFile($file)){
            echo '<script>alert("Impossible de créer le fichier: ' .$dir. DIRECTORY_SEPARATOR .$_GET['PageName']. '.xml");</script>';
        }
        ...
        if(\Administration\app\Fopen::writeFile($file, $textxml)){
            echo '<script>alert("Impossible d\'écrire le fichier: ' .$dir. DIRECTORY_SEPARATOR .$_GET['PageName']. '.xml");</script>';
        }
